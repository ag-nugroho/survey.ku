    ...
    if ($result_survey) {
        ...
        foreach ($_POST as $key => $value) {
            if (strpos($key, 'pertanyaan') === 0) {
                ...
                // Masukkan jawaban ke tabel t_jawaban_industri
                $query_jawaban = "INSERT INTO t_jawaban_industri (user_id, survey_id, survey_kategori_id, soal_id, jawaban) VALUES ('$user_id', '$survey_id', '$survey_kategori_id', '$soal_id', '$jawaban')";
                $result_jawaban = $conn->query($query_jawaban);

                if ($result_jawaban) {
                    echo "Jawaban untuk soal $soal_id berhasil dimasukkan ke t_jawaban_industri!<br>";
                } else {
